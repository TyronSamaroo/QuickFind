...
display_usage() {
    echo "Usage: $0 {fs|ff} [arguments]"
    echo "
    QuickFind is a powerful file searching and filtering tool inspired by Neovim's Telescope plugin.

    Commands:
      fs - Search files with options for file type and pattern.
      ff - Search within directories with options for language and depth.

    Examples:
      $0 fs py keyword        # Search for 'keyword' in Python files
      $0 ff c js 1            # Search for JavaScript files in 'Documents/Code' with depth 1

    Use '$0 {command} -h' for more information on a specific command.
    "
} 
